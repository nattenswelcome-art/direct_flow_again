...
class MockWordstatProvider(WordstatProvider):
    ...
    - Частотность зависит от хеша ключевого слова (детерминированно)
    - Возвращает только введённые ключи (без расширения)

    Примечание: Для реальных связанных запросов используйте YandexWordstatProvider.
    """
    ...
    async def get_keywords(
        ...
        Returns:
            Список KeywordRow с mock-данными (только введённые ключи)
        """
        results = []
